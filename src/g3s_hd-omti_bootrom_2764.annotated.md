...
- ~~No hard-disk `.hdv` image with a real boot sector has been built or tested against this yet — the `IX=4200h` hand-off is understood architecturally but has not been exercised end-to-end.~~ **Done, 2026-08-01, by `CalvaDos` (not this repo)**: a real OMTI `.hdv` with a working boot sector at `4200h` has been built and exercised end-to-end against this exact ROM — `run-hdboottest.sh` reaches PASS. See the correction above and `CalvaDos/docs/development/boot-patch-inventory.md`.
